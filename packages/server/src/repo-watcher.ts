...
const REPO_WATCH_DEAD_POLL_MS = 3_000;
...
function makeRepoWatcherLoop(options: {
	...
	return Effect.gen(function* () {
		const watcherDeadRef = yield* Ref.make(false);

		const triggerQueue = yield* Queue.sliding<void>(1);
		const pollWakeQueue = yield* Queue.sliding<void>(1);

		const triggerRefresh = Queue.offer(triggerQueue, undefined).pipe(
			Effect.asVoid,
		);
		const wakePollLoop = Queue.offer(pollWakeQueue, undefined).pipe(
			Effect.asVoid,
		);
		...
		const fsWatchFiber = yield* options.fs
			...
			.pipe(
				...
				Effect.catchAllCause((cause) =>
					Ref.set(watcherDeadRef, true).pipe(
						Effect.zipRight(triggerRefresh),
						Effect.zipRight(wakePollLoop),
						Effect.zipRight(
							Effect.logWarning(
								`[repo-watcher] filesystem watch stream failed for ${options.repoRoot}; switching to ${REPO_WATCH_DEAD_POLL_MS}ms polling: ${Cause.pretty(cause, { renderErrorCause: true })}`,
							),
						),
					),
				),
		...
		const safetyPollFiber = yield* pipe(
			Effect.forever(
				Effect.gen(function* () {
					const pollMs = (yield* Ref.get(watcherDeadRef))
						? REPO_WATCH_DEAD_POLL_MS
						: REPO_WATCH_SAFETY_POLL_MS;

					yield* Effect.raceFirst(
						Effect.sleep(`${pollMs} millis`),
						Queue.take(pollWakeQueue),
					);

					yield* Queue.takeAll(pollWakeQueue);
		...
		return yield* Effect.forever(
			Effect.gen(function* () {
				...
				const maybeNextSnapshot = yield* computeRepoSnapshot(
					options.repoRoot,
				).pipe(
